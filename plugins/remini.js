...
async function req(url, method = "GET", data = null, params = null, head = null, customHeaders = null) {
  try {
    let pUrl;
    if (params) {
    ...
    return await fetch(pUrl, {
      method,
      headers: head ? head : (customHeaders ? customHeaders : getHeadersList()),
      ...(data ? {
        body: data
      } : {})
    })
...
async function _init(headList) {
  const rs = await req(BASE_URL + URL_USER, "POST", null, null, null, headList);
  const jsn = await rs.json();
  headList["Authorization"] = `Bearer ${jsn.access_token}`;
  return true;
}

async function _upload(settings, headList) {
  const hh = await req(BASE_URL + URL_BULK, "POST", JSON.stringify(BULK_PAYLOAD(settings)), null, null, headList);
  const jsn = await hh.json();
  return jsn;
}

async function _approval(data, settings, headList) {
  const hh = await req(BASE_URL + URL_APPROVAL.replace("BULK_UPLOAD_ID", data.bulk_upload_id), "POST", JSON.stringify(BULK_PAYLOAD(settings)), null, null, headList);
  return hh.status;
}

async function _approvalWm(headWm) {
  const hh = await req(BASE_URL_WM + URL_REMOVE_WM, "OPTIONS", null, null, headWm);
  return hh.status;
}

async function _send(data, buffer, headList) {
  const hd = { ...headList };
  hd["content-type"] = "image/jpeg";
  hd["x-goog-custom-time"] = data.task_list[0].upload_headers["x-goog-custom-time"];
  const jj = await req(data.task_list[0].upload_url, "PUT", buffer, null, hd);
  return jj.status;
}

async function _task(data, headList) {
  const hd = { ...headList };
  hd["content-type"] = "application/json";
  const jj = await req(BASE_URL + URL_TASK + data.bulk_upload_id, "GET", null, null, hd);
...
async function Remove(image) {
  try {
    ...
    if (Buffer.isBuffer(image)) {
      buffer = image;
    } else if (/https\:\/\/|http\:\/\//i.test(image)) {
...
function Remini(buffer, settings) {
  return new Promise(async (resolve, reject) => {
    try {
      const headList = getHeadersList();
      await _init(headList);
      const data = await _upload(settings, headList);
      const sendStatus = await _send(data, buffer, headList);
      if (sendStatus !== 200) {
        return reject(new Error("Failed to send image to Remini."));
      }
      const status = await _approval(data, settings, headList);
      if (status !== 202) {
        return reject(new Error("Remini task approval failed."));
      }
      ...
      while (true) {
        const jdata = await _task(data, headList);
        if (jdata.task_list && jdata.task_list[0] && jdata.task_list[0].status === "completed") {
          wm = jdata;
          break;
        } else if (jdata.task_list && jdata.task_list[0] && jdata.task_list[0].status === "failed") {
          return reject(new Error("Remini enhancement failed."));
        }
        await _delay(2000);
      }

      const originalOutputUrl = wm.task_list[0].result.outputs[0].url;

      if (wm.task_list[0].result.outputs[0].has_watermark) {
        try {
          const rm = await Remove(originalOutputUrl);
          if (rm && rm.output && rm.output[0]) {
            no_wm = rm.output[0];
          } else {
            no_wm = originalOutputUrl;
          }
        } catch (_) {
          no_wm = originalOutputUrl;
        }
...
cmd({
  pattern: 'remini',
  alias: ['hd', 'upscale', 'enhance'],
  desc: 'Enhance image quality using Remini AI (Owner only/Public)',
  category: 'tools',
  react: '📸',
  ...
  async (conn, mek, m, {  }) => {
    ...
    try {
      ...
      const mediaPath = await conn.downloadAndSaveMediaMessage(quoted, 'remini_temp');
      const fs = require('fs');
      ...
      const result = await Remini(buffer, SETTINGS);

      if (!result || !result.no_wm) {
        throw new Error("Enhancement returned empty result.");
      }

      const enhancedImage = result.no_wm;

      await conn.sendMessage(from, {
        image: { url: enhancedImage },
        caption: `╭━━━〔 *✨ ɴᴏᴠᴀ_x ʀᴇᴍɪɴɪ* 〕━━━┈\n┃\n┃ ✅ *Image enhanced successfully!*\n┃ 🖼️ *Quality:* HD / Ultra HD\n┃\n╰━━━━━━━━━━━━━━━┈\n> ${config.BOT_FOOTER}`,
        contextInfo: {
          forwardingScore: 999,
          isForwarded: true,
          forwardedNewsletterMessageInfo: {
      ...
      console.error('REMINI CMD ERROR:', err);
      await conn.sendMessage(from, { react: { text: '❌', key: mek.key } });
      await conn.sendMessage(from, {
        text: `╭━━━〔 *❌ ɴᴏᴠᴀ_x ᴍɪɴɪ* 〕━━━┈\n┃ *Remini enhancement failed!*\n┃ 🛠 *Error:* ${err.message}\n╰━━━━━━━━━━━━━━━┈`,
        contextInfo: {
          forwardingScore: 999,
          isForwarded: true,
          forwardedNewsletterMessageInfo: {
