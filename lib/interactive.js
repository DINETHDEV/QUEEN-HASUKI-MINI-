...
/**
 * Convenience error-message helper.
 * Plugins that import { errMsg } from '../lib/interactive' will get this.
 * Usage: await errMsg(conn, from, mek, 'Something went wrong')
 *
 * @param {object} conn   Baileys WASocket
 * @param {string} from   Target chat JID
 * @param {object} mek    Quoted message (may be null)
 * @param {string} text   Error description text
 */
async function errMsg(conn, from, mek, text) {
    try {
        ...
        logger.warn('[Interactive:errMsg] Failed to send error message:', e.message);
    }
}

/**
 * Single select/list helper for native flow single_select
 * @param {string} title Button text
 * @param {object[]} sections Sections list
 */
function select(title, sections) {
    return {
        name: 'single_select',
        buttonParamsJson: JSON.stringify({
            title: title,
